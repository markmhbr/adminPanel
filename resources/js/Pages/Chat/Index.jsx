...
export default function Chat({ auth, chat, messages: initialMessages }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight italic uppercase">Pusat Bantuan & Live Chat</h2>}
        >
                ...
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden flex flex-col h-[600px]">
                        ...
                        <div className="px-8 py-6 border-b border-slate-50 flex items-center gap-4 bg-slate-50/50">
                            ...
                            <div>
                                <h3 className="text-sm font-black text-slate-900 italic uppercase leading-none">Support Team Simak Buy</h3>
                                <p className="text-[10px] font-bold text-emerald-500 mt-1 italic uppercase tracking-widest flex items-center gap-1">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-6">
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.sender_id === auth.user.id ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[70%] p-5 rounded-[1.5rem] text-sm font-bold italic ${msg.sender_id === auth.user.id ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-100 text-slate-700 rounded-tl-none'}`}>
                                        {msg.message}
                        ...
                        <div className="p-6 border-t border-slate-50 bg-slate-50/30">
                            <form onSubmit={sendMessage} className="flex gap-4">
                                <input 
                                    ...
                                    className="flex-1 px-6 py-4 bg-white border-2 border-transparent rounded-2xl focus:border-indigo-600 focus:ring-0 transition-all font-bold italic text-sm shadow-inner"
                                />
                                <button 
                                    type="submit"
                                    className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all italic flex items-center gap-2"
                                >
